...
// authorization for google drive account
    $(document).on('click','#authorize-btn-gdrive',function () {
        var gdrive_auth_code = $('#auth-token-gdrive').val();
        if (gdrive_auth_code == '') {
            $('.auth-message.error').text(admin_sendfiles.add_token).fadeIn();
        }else{
            $('.auth-message').empty();
            ...
            $.ajax({
                ...
                type : 'post',
                data : {
                    action : 'sendfiles_authenticate_gdrive',
